...
int main(int ac, char *av[]) {
  ...
  const std::string peram_path = "/data/LapHs/contraction_Markus/test_data/peram/up";
  const quark q("u", nb_peram, "B", nb_dil_T, "I", nb_dil_E, "F", 4, 0, peram_path);
  ...
  for (ssize_t config_i = start_config; config_i <= end_config;
    ...
    for (int rnd_vec_i = 0; rnd_vec_i < nb_rnd_vec; ++rnd_vec_i) {
      ...
      char temp2[200];
      sprintf(temp2,
              "randomvector.rndvecnb%02d.u.nbev%04zu.%04zu",
              rnd_vec_i,
              nb_ev,
              config_i);
    ...
    perambulators.read_perambulators_from_separate_files(
        Lt, nb_ev, std::vector<quark>(1, q), peram_names);
    // read random vectors -----------------------------------------------------
