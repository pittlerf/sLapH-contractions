...
int main() {
  ...
  std::string path_ev_out(
      "/hiskp4/helmes/contractions/test/derivative_operators/covariant_shift");
  ...
  auto const path_in = (boost::format("%s/eigenvectors.0400.") % path_ev_in).str();
  ...
  for (ssize_t t = 0; t < 24; ++t) {
    ...
    //    Eigen::MatrixXcd disp_eigvecs =
    //    gauge.displace_eigenvectors(V_t[t],t,displacements,1);
