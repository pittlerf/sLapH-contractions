  ...
  for(ssize_t t = 0; t < Lt; ++t){
    ...
    for(const auto& op : operator_lookuptable.vdaggerv_lookup){
      ...
      if(op.id != id_unity){
        ...
        std::string dummy = filename_vdaggerv + ".p_" + 
...
int main (int ac, char* av[]) {
  ...
  for(ssize_t config_i  = start_config; config_i <= end_config; 
    ...
    const std::string filename_ev = 
         "/data/LapHs/contraction_Markus/test_data/ev/eigenvectors." + 
         std::string(cnfg);
    const std::string pathname_vdaggerv = 
         "/data/LapHs/contraction_Markus/test_data/operators/cnfg" +
         std::string(cnfg) + "/";
    const std::string filename_vdaggerv = 
         "operators." + 
         std::string(cnfg);
    build_and_write_vdaggerv(Lt, Lx, Ly, Lz, nb_ev, operator_lookuptable,
                             filename_ev, pathname_vdaggerv, filename_vdaggerv);
