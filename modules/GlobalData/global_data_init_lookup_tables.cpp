...
static void init_lookup_corr(const Correlator_list& correlator_list, 
  ...
  for(const auto& corr_list : correlator_list)
    used_operator_IDs.insert(used_operator_IDs.end(), 
                          corr_list.operator_numbers.begin(), 
                          corr_list.operator_numbers.end());
  
  sort(used_operator_IDs.begin(), used_operator_IDs.end());
  used_operator_IDs.erase(std::unique(used_operator_IDs.begin(), 
                                   used_operator_IDs.end()),
                       used_operator_IDs.end());
  ...
  for(const auto op : lookup_corr){
    std::cout << op.id;
    for(const auto& ga : op.gamma)
    std::cout << " " << ga;
    for(const auto& dis : op.dis3)
      std::cout << " " << dis;
    for(const auto& mom : op.p3)
      std::cout << " " << mom;
    std::cout << "\t" << op.id_vdv << " " << op.first_vdv << " " 
              << op.negative_momentum << " " << op.id_rvdvr << std::endl;
  }
...
void GlobalData::init_lookup_tables() {
  ...
  // loop over the correlator list - lookup table are build recursively
  for (const auto& corr_list : correlator_list) {
