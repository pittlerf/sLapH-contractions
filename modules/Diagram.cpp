...
void C2c::assemble_impl(std::vector<Complex> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    auto const &x = q.trQ0Q2.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[0]);
...
void C20::assemble_impl(std::vector<Complex> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    auto const &x = q.trQ1Q1.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[0]);
    c[i] += std::accumulate(std::begin(x), std::end(x), Complex(0.0, 0.0)) /
            static_cast<double>(x.size());
  }
}
...
//    assert(slice_pair.sink() < q.trQ1.tr.shape()[1]);
    ...
    c[i] += inner_product(
        q.trQ1Q1.at({slice_pair.source(),slice_pair.source()},c_look.lookup[0]),
        q.trQ1.tr[c_look.lookup[1]][slice_pair.sink()]);
  }
  LT_DIAGRAMS_STOP;
...
void C4cD::assemble_impl(std::vector<ComplexProduct> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    c[i] +=
        inner_product(q.trQ0Q2.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[0]),
                      q.trQ0Q2.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[1]));
  }
  LT_DIAGRAMS_STOP;
...
void C40D::assemble_impl(std::vector<ComplexProduct> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    c[i] +=
        inner_product(q.trQ1Q1.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[0]),
                      q.trQ1Q1.at({slice_pair.source(),slice_pair.sink()},c_look.lookup[1]));
  }
  LT_DIAGRAMS_STOP;
...
void C4cV::assemble_impl(std::vector<ComplexProduct> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    c[i] += inner_product(
        q.trQ0Q2.at({slice_pair.source(),slice_pair.source()},c_look.lookup[0]),
        q.trQ0Q2.at({slice_pair.sink(),slice_pair.sink()},c_look.lookup[1]));
  }
  LT_DIAGRAMS_STOP;
...
void C40V::assemble_impl(std::vector<ComplexProduct> &c,
  ...
  for (int i = 0; i != ssize(corr_lookup()); ++i) {
    ...
    c[i] += inner_product(
        q.trQ1Q1.at({slice_pair.source(),slice_pair.source()},c_look.lookup[0]),
        q.trQ1Q1.at({slice_pair.sink(),slice_pair.sink()},c_look.lookup[1]));
  }
  LT_DIAGRAMS_STOP;
